...
from pdearena.modules.conditioned.oned_unet import Unet as CondUNet
...
class EffParametricUNet(LightningModule):
    """Parameter-conditioned U-Net for eff-physics-learn-dataset parametric modality (burgers)."""
    ...
    def __init__(
        ...
        *,
        ...
        n_params: int = 3,  # A, k, nu for burgers
        hidden_channels: int = 64,
        activation: str = "gelu",
        norm: bool = True,
        lr: float = 1e-3,
        weight_decay: float = 0.0,
        n_dims: int = 2,  # Spatial dimensions: 2 for 2D problems, 3 for 3D problems
        ...
        # Use the generic conditioned UNet (despite the module name), with configurable n_dims.
        # This supports multi-parameter conditioning via `param_conditioning="scalar_N"`.
        # n_dims=2 for 2D problems (burgers, allen_cahn, etc.), n_dims=3 for 3D problems (helmholtz3D, flow_mixing).
        self.model = CondUNet(
    ...
    def on_test_end(self) -> None:
        ...
        if self.hparams.plot_out_path and self.trainer.is_global_zero:
            ...
            def _plot_rows(rows, out_path: Path, title_prefix: str):
                ...
                # Handle 3D data by taking middle slices for min/max calculation
                ...
                for r, ex in enumerate(rows):
                    ...
                    if gt.ndim == 3:
                        # Take middle slice along the last dimension
                        slice_idx = gt.shape[2] // 2
                        gt = gt[:, :, slice_idx]
                        pred = pred[:, :, slice_idx]
                    elif gt.ndim != 2:
